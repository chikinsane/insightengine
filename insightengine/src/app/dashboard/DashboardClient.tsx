          ...
          <div className="max-w-[520px]">
            ...
            <div className="flex flex-wrap gap-2">
              {EXAMPLE_QUERIES.map(q => (
                <button key={q} className="px-3 py-1.5 text-xs text-white/30 border border-white/[0.07] rounded-full hover:border-violet-500/40 hover:text-violet-300 hover:bg-violet-500/5 transition-all">
                  {q}
                </button>
              ))}
            </div>
            ...
            <div className="flex gap-8 mt-10 pt-8 border-t border-white/[0.06]">
              {STATS.map(s => (
                <div key={s.label}>
                  <div className="text-2xl font-bold bg-gradient-to-b from-white to-white/60 bg-clip-text text-transparent">{s.value}</div>
